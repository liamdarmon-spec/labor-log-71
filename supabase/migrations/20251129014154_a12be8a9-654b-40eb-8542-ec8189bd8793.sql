...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_work_schedules_scheduled_date
  ON public.work_schedules (scheduled_date);

-- By worker + scheduled_date (used in schedule views and conflict checks)
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_work_schedules_worker_date
  ON public.work_schedules (worker_id, scheduled_date);

-- By status + scheduled_date (used by sync/cleanup processes)
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_work_schedules_status_date
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_labor_pay_runs_created_at
  ON public.labor_pay_runs (created_at);

-- By payment_date + status (used when counting 'paid' runs in a period)
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_labor_pay_runs_payment_date_status
  ON public.labor_pay_runs (payment_date, status);

-- By payer_company_id (filter by company in Pay Center)
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_labor_pay_runs_payer_company
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_labor_pay_run_items_pay_run_id
  ON public.labor_pay_run_items (pay_run_id);

CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_labor_pay_run_items_time_log_id
