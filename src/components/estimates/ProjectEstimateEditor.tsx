...
export const ProjectEstimateEditor: React.FC<EstimateEditorProps> = ({
  ...
  const subtotal = globalTotals.labor + globalTotals.subs + globalTotals.materials + globalTotals.other;
  const tax = 0;
...
const ItemRow: React.FC<ItemRowProps> = ({ blockId, item, updateItem, deleteItem }) => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-center gap-1">
          <select
            ...
            value={item.category}
            onChange={(e) => updateItem(blockId, item.id, { category: e.target.value as BudgetCategory })}
          >
        ...
        <input
          ...
          value={item.quantity}
          onChange={(e) => updateItem(blockId, item.id, { quantity: Number(e.target.value || 0) })}
        />
        ...
        <input
          ...
          onChange={(e) => updateItem(blockId, item.id, { unit_price: Number(e.target.value || 0) })}
        />
        ...
        <input
          ...
          onChange={(e) => updateItem(blockId, item.id, { markup_percent: Number(e.target.value || 0) })}
        />
      ...
      {/* Mobile layout */}
      <div className={cn("lg:hidden p-3 mb-2 rounded-lg border border-border", invalid && "bg-destructive/10")}>
...
const CategorySummaryCard: React.FC<{ totals: Record<BudgetCategory, number> }> = ({ totals }) => (
