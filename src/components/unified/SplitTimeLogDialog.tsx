/**
 * SplitTimeLogDialog - Multi-project split for time_logs
 * 
 * CANONICAL: Calls split_time_log_for_multi_project RPC
 * 
 * The RPC function:
 * - Updates the original time_log entry with first project split
 * - Creates new time_log entries for additional projects
 * - Syncs with linked schedules if they exist
...
interface SplitTimeLogDialogProps {
  isOpen: boolean;
  onClose: () => void;
  timeLogId: string;
  workerName: string;
...
export function SplitTimeLogDialog({
  isOpen,
  onClose,
  timeLogId,
  workerName,
  ...
  const addEntry = () => {
    setEntries([...entries, { project_id: '', trade_id: '', cost_code_id: '', hours: '', notes: '' }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Call the split time log function
      const { data, error } = await supabase.rpc('split_time_log_for_multi_project', {
        p_original_time_log_id: timeLogId,
      ...
      toast({
        title: 'Success',
        description: `Time log split into ${entries.length} entries`,
      });
      ...
      toast({
        title: 'Error',
        description: error.message || 'Failed to split time log',
        variant: 'destructive'
      });
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Split Time Log for Multiple Projects</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              ...
              <br />
              Original hours: <strong>{originalHours}h</strong>
              <br />
              This time log will be split into {entries.length} separate entries.
            </AlertDescription>
          </Alert>
