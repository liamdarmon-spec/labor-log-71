 ...
 * Anything that used to read from project_financials_snapshot
 * now just uses V3 directly.
 */

// Old name some components might still import
export function useProjectFinancialsSnapshot(projectId: string) {
  return useProjectFinancialsV3(projectId);
}

// Another legacy name that might exist
